...
if(!isset($_SESSION['uCode'])){
    ...
    if(!empty($_GET) && isset($_GET['oCode'])){
        ...
        if ($stmt = mysqli_prepare($conn, "SELECT PT.outcome, P.message FROM PRIZE_TRANSACTION PT, PRIZE P WHERE oCode = ? AND uCode = ? AND P.pCode = PT.pCode")) {
            ...
            if (!mysqli_stmt_execute($stmt))
                noRollBackError($conn);
            if(!mysqli_stmt_bind_result($stmt, $outcome, $message))
                noRollBackError($conn);
